  ...
  return (
    ...
    <header className="bg-white dark:bg-slate-800 border-b dark:border-slate-700 sticky top-0 z-40 transition-colors">
      ...
      <nav className="hidden md:flex text-sm text-slate-700 dark:text-slate-200 gap-10">
        <Link href="/" className="hover:text-accent-500 transition-colors text-lg">{t.nav.home}</Link>
        <Link href="/projects" className="hover:text-accent-500 transition-colors text-lg">{t.nav.projects}</Link>
        <Link href="/departments" className="hover:text-accent-500 transition-colors text-lg">{t.nav.departments}</Link>
        <Link href="/management" className="hover:text-accent-500 transition-colors text-lg">{t.nav.management}</Link>
        <Link href="/contact" className="hover:text-accent-500 transition-colors text-lg">{t.nav.contact}</Link>
        <Link href="/partners" className="hover:text-accent-500 transition-colors text-lg">{t.nav.partners}</Link>
      </nav>
      ...
      {open && (
        <div className="md:hidden border-t border-slate-200 dark:border-slate-700">
          <div className="container mx-auto px-6 py-3 flex flex-col gap-3 text-slate-700 dark:text-slate-200">
            <Link onClick={() => setOpen(false)} href="/" className="py-1">{t.nav.home}</Link>
            <Link onClick={() => setOpen(false)} href="/projects" className="py-1">{t.nav.projects}</Link>
            <Link onClick={() => setOpen(false)} href="/departments" className="py-1">{t.nav.departments}</Link>
            <Link onClick={() => setOpen(false)} href="/management" className="py-1">{t.nav.management}</Link>
            <Link onClick={() => setOpen(false)} href="/contact" className="py-1">{t.nav.contact}</Link>
            <Link onClick={() => setOpen(false)} href="/partners" className="py-1">{t.nav.partners}</Link>
          </div>
        </div>
      )}
    </header>
